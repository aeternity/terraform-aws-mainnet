...
module "aws_deploy-main-eu-north-1" {
  ...
  # m6i.large keeps 8GiB mem (vs c6i.large's 4GiB) to leave headroom for a possible future
  # MPT read cache.
...
module "aws_deploy-main-us-west-2" {
  ...
  # already hit CPUCreditBalance=0 (hard-throttled) this week.
  instance_type  = "m6i.large"
...
module "aws_deploy-main_mon-eu-north-1" {
  ...
  # 2026-07-20: t3.medium -> m6i.large. Declining CPUCreditBalance trend (not yet at 0);
  # fixed before it hits the same wall as uat_mon.
  instance_type  = "m6i.large"
  instance_types = ["m6i.large", "c6i.large", "c5.large", "m5.large"]
  ami_name       = "aeternity-ubuntu-22.04-v1709639419"
